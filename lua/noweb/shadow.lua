...
-- @NAME@ stands for the (escaped) identifier.  The default language
-- set mirrors the languages noweb ships autodefs filters for, and
-- the patterns port each recognizer's rules.
local sh_definitions = { '^%s*@NAME@%s*%(%s*%)',
                         ...
                         '%f[%w]read%s+@NAME@%f[%W]',
                         '%f[%w]@NAME@=', '%f[%w]@NAME@%+=' }
local c_definitions = { '^%s*#%s*define%s+@NAME@%f[%W]',
                        '^[%w_][%w_%s%*&<>,:]-%f[%w]@NAME@%s*%(',
                        '%f[%w]struct%s+@NAME@%f[%W]',
                        '%f[%w]enum%s+@NAME@%f[%W]',
                        '%f[%w]union%s+@NAME@%f[%W]',
                        '%f[%w]typedef[^;]*%f[%w]@NAME@%s*;' }
local defaults = {
  ...
  lua    = { leader = '--', filetype = 'lua',
             ...
             definitions = { 'function%s+@NAME@%f[%W]',
                             ...
                             '^%s*@NAME@%s*=',
                             '%f[%w]for%s+@NAME@%f[%W]' } },
  sh     = { leader = '#', filetype = 'sh', definitions = sh_definitions,
             lsp = { 'bash-language-server', 'start' } },
  zsh    = { leader = '#', filetype = 'zsh', definitions = sh_definitions,
             lsp = { 'bash-language-server', 'start' } },
  make   = { leader = '#', filetype = 'make',
             definitions = { '^@NAME@%s*:' } },
  c      = { leader = '//', filetype = 'c', definitions = c_definitions,
             lsp = { 'clangd' } },
  cpp    = { leader = '//', filetype = 'cpp', lsp = { 'clangd' },
             definitions = vim.list_extend(
               { '%f[%w]class%s+@NAME@%f[%W]',
                 '%f[%w]namespace%s+@NAME@%f[%W]',
                 '%f[%w][%w_]+::@NAME@%s*%(' },
               c_definitions) },
  haskell = { leader = '--', filetype = 'haskell',
              lsp = { 'haskell-language-server-wrapper', '--lsp' },
              definitions = { '^@NAME@%s*::', '^@NAME@[%s,].*::',
                              '%f[%w]data%s+@NAME@%f[%W]',
                              '%f[%w]newtype%s+@NAME@%f[%W]',
                              '%f[%w]type%s+@NAME@%f[%W]',
                              '%f[%w]class%s+@NAME@%f[%W]',
                              '^@NAME@%f[%W]' } },
  java   = { leader = '//', filetype = 'java', lsp = { 'jdtls' },
             definitions = { '%f[%w]class%s+@NAME@%f[%W]',
                             '%f[%w]interface%s+@NAME@%f[%W]',
                             '%f[%w]enum%s+@NAME@%f[%W]',
                             '%f[%w]record%s+@NAME@%f[%W]',
                             '^%s*[%w<>%[%],%s]-%f[%w]@NAME@%s*%(',
                             '^%s*[%w<>%[%],%s]-%f[%w]@NAME@%s*[=;]' } },
  rust   = { leader = '//', filetype = 'rust', lsp = { 'rust-analyzer' },
             definitions = { '%f[%w]fn%s+@NAME@%f[%W]',
                             '%f[%w]struct%s+@NAME@%f[%W]',
                             '%f[%w]enum%s+@NAME@%f[%W]',
                             '%f[%w]trait%s+@NAME@%f[%W]',
                             '%f[%w]mod%s+@NAME@%f[%W]',
                             '%f[%w]union%s+@NAME@%f[%W]',
                             '%f[%w]type%s+@NAME@%f[%W]',
                             '%f[%w]const%s+@NAME@%f[%W]',
                             '%f[%w]static%s+@NAME@%f[%W]',
                             '%f[%w]let%s+@NAME@%f[%W]',
                             '%f[%w]let%s+mut%s+@NAME@%f[%W]',
                             'macro_rules!%s*@NAME@%f[%W]' } },
  tex    = { leader = '%', filetype = 'tex', lsp = { 'digestif' },
             definitions = { '\\def%s*\\@NAME@%f[%W]',
                             '\\newcommand%*?%s*{?\\@NAME@%f[%W]',
                             '\\renewcommand%*?%s*{?\\@NAME@%f[%W]',
                             '\\providecommand%*?%s*{?\\@NAME@%f[%W]',
                             '\\newenvironment%s*{@NAME@}',
                             '\\newtheorem%s*{@NAME@}' } },
  icon   = { leader = '#', filetype = 'icon',
             definitions = { '%f[%w]procedure%s+@NAME@%f[%W]',
                             '%f[%w]record%s+@NAME@%f[%W]',
                             '%f[%w]global%s[%w%s,]-%f[%w]@NAME@%f[%W]' } },
  pascal = { leader = '//', filetype = 'pascal',
             definitions = { '%f[%w][Pp]rocedure%s+@NAME@%f[%W]',
                             '%f[%w][Ff]unction%s+@NAME@%f[%W]',
                             '%f[%w][Cc]onstructor%s+@NAME@%f[%W]',
                             '^%s*@NAME@%s*=',
                             '^%s*@NAME@%s*:' } },
  yacc   = { leader = '//', filetype = 'yacc',
             definitions = { '^@NAME@%s*:',
                             '^%%token[%w%s<>%.]-%f[%w]@NAME@%f[%W]',
                             '^%%type[%w%s<>%.]-%f[%w]@NAME@%f[%W]' } },
}
-- The filetype database answers 'plaintex' for a bare .tex name.
defaults.plaintex = vim.tbl_extend('force', defaults.tex,
                                   { filetype = 'plaintex' })
...
local function ensure_shadow(nwbuf, root, lang, cfg)
  ...
  if not (sh and vim.api.nvim_buf_is_valid(sh.buf)) then
    ...
    -- Defaults name a server per language; only attach the ones that
    -- are actually installed, so the defaults are harmless everywhere.
    if cfg.lsp and vim.fn.executable(cfg.lsp[1]) == 1 then
      vim.lsp.start({
        name = 'noweb-' .. lang,
        cmd = cfg.lsp,
